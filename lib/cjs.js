  ...
  const bakeRoute = (parentConfig, key) => {
    ...
    if (isObject(routeBase)) {
      if (!isRequestConfig(routeBase)) {
         ...
         * @type { OmnicRequest }
         */
        const requestFunction = _async(function (requestConfig) {
          if (routeConfigIsPath(requestConfig)) requestConfig = { path: requestConfig };

          if (requestConfig) {
            config$$1 = mergeConfigs(parentConfig, _this.config, requestConfig);
          }
           ...
           * @type { Response }
           */
          return _await(_this.adapter.request(path, finalConfig), function (result) {

            let response;
            return _invoke(function () {
              if (afterEach) {
                return _await(afterEach(result), function (_afterEach) {
                  response = _afterEach;
                });
              } else {
                response = result;
...
exports.omnicFactory = omnicFactory;
